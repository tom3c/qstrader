...
def test_get_account_cash_balance():
    ...
    tcb1 = dict(
        zip(currency_list, [0.0] * len(currency_list))
    )
    tcb1["USD"] = 1000.0
    assert sbcb1 == tcb1
...
def test_get_account_cash_balance_diff_base_subs_witdrawals():
    ...
    tcb = dict(zip(currency_list, [0.0] * len(currency_list)))
    tcb["AUD"] = 2000.0
    assert sbcb == tcb

    sbmc.subscribe_funds_to_account(16500.25, currency='USD')
    sbcb = sbmc.get_account_cash_balance()
    tcb["USD"] = 16500.25
    assert sbcb == tcb

    sbmc.subscribe_funds_to_account(500.25, currency='AUD')
    sbmc.subscribe_funds_to_account(34500.12, currency='HKD')
    sbmc.subscribe_funds_to_account(1500.25, currency='EUR')
    sbmc.subscribe_funds_to_account(499.75, currency='USD')
    tcb["AUD"] = 2500.25
    tcb["EUR"] = 1500.25
    tcb["HKD"] = 34500.12
    tcb["USD"] = 17000.00
    ...
    # Raising ValueError for lack of cash
    with pytest.raises(ValueError):
        sbmc.withdraw_funds_from_account(2e6, "HKD")

    # Correctly setting cash_balances for a positive amount
    sbmc.withdraw_funds_from_account(2500.25, currency='AUD')
    sbmc.withdraw_funds_from_account(10.00, currency='HKD')
    sbmc.withdraw_funds_from_account(10.00, currency='EUR')
    sbmc.withdraw_funds_from_account(10.00, currency='USD')
    tcb["AUD"] = 0.00
    tcb["EUR"] = 1490.25
    tcb["HKD"] = 34490.12
    tcb["USD"] = 16990.00
