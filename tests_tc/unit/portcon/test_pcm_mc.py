...
def test_obtain_equity_asset_list(description, port_dict, uni_assets, expected):
    """
    ...
    broker = Mock()
    broker.get_portfolio_equity_as_dict.return_value = port_dict

    universe = Mock()
    universe.get_equity_assets.return_value = uni_assets

    order_sizer = Mock()
    optimiser = Mock()

    pcm = PortfolioConstructionModel_MC(
        broker, port_id, universe, order_sizer, optimiser
    )

    result = pcm._obtain_equity_asset_list(SENTINEL_DT)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {},
            [
                Order_MC('STOCK_ORDER', SENTINEL_DT, 'EQ:ABC', 100, currency='USD'),
                Order_MC('STOCK_ORDER', SENTINEL_DT, 'EQ:DEF', 250, currency='USD')
            ]
        ),
        (
            ...
            [
                Order_MC('STOCK_ORDER', SENTINEL_DT, 'EQ:ABC', -345, currency='USD'),
                Order_MC('STOCK_ORDER', SENTINEL_DT, 'EQ:DEF', -250, currency='USD')
            ]
        ),
        (
            ...
            [
                Order_MC('STOCK_ORDER', SENTINEL_DT, 'EQ:ABC', 123, currency='USD'),
                Order_MC('STOCK_ORDER', SENTINEL_DT, 'EQ:DEF', 456, currency='USD'),
                Order_MC('STOCK_ORDER', SENTINEL_DT, 'EQ:GHI', -217, currency='USD'),
                Order_MC('STOCK_ORDER', SENTINEL_DT, 'EQ:JKL', -48, currency='USD')
            ]
        ),
        (
            ...
            [
                Order_MC('STOCK_ORDER', SENTINEL_DT, 'EQ:ABC', 123, currency='USD'),
                Order_MC('STOCK_ORDER', SENTINEL_DT, 'EQ:DEF', 239, currency='USD'),
                Order_MC('STOCK_ORDER', SENTINEL_DT, 'EQ:GHI', -48, currency='USD')
            ]
        ),
        (
            ...
            [
                Order_MC('STOCK_ORDER', SENTINEL_DT, 'EQ:ABC', -94, currency='USD'),
                Order_MC('STOCK_ORDER', SENTINEL_DT, 'EQ:DEF', 408, currency='USD')
            ]
        )
    ]
)
def test_generate_rebalance_orders(
    ...
    equity_assets = [('EQ:ABC', 'USD'), ('EQ:DEF', 'USD'), ('EQ:GHI', 'USD'), ('EQ:JKL', 'USD')]
